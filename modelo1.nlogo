...
techs-own [K A T F Mk M]
firms-own [K A T F L Mk
          W Q P C M D R V
          Profit Prate
          t1 t2 t3]
...
to entry
  ...
  if random-normal 0.5 0.2 < 0.9 [stop]

  ask one-of techs [
    set A sum [M * A] of firms
    set T sum [M * T] of firms
    set F sum [M * F] of firms

    set Mk 1.1
    set M (1 / count firms)
    set K sum [M * K] of firms
...
to Global-Competitiveness
   set Ctotal (sum [C * M] of firms)
end


to Market-Share
  ask firms [ set M (M * (1.0 + Speed * ((C / Ctotal) - 1.0))) ]

  let total-Market-Share sum [M] of firms ; g1
  ask firms [set M (M / total-Market-Share)]
end

to Demand
    set Y (S + Kcons + Ktotal)
    set S 0.0
    set Kcons 0.0
    set Ktotal 0.0
end
...
to Sales-Profit
    set D (M * Y / P)
    if D = Q [set R (P * D) set V 0.0     set S (S + 0.0)          ]
    if D > Q [set R (P * Q) set V 0.0     set S (S + (P * (D - Q)))]
    if D < Q [set R (P * D) set V (Q - D) set S (S + 0.0)          ]
    set Profit (R - K)
    set Prate (100.0 * Profit / K)
end

to ShareHolders
    ifelse (R > 0.05 * K) [set Kcons (Kcons + 0.05 * K) set K (R - 0.05 * K)]
                        [set Kcons R set K 0]
end

to plot-information
    set-current-plot "Capital X Tec"
        clear-plot
    ...
    set-current-plot "População"
        create-temporary-plot-pen "pop"
        plot (count turtles with [breed = firms])
end
@#$#@#$#@
GRAPHICS-WINDOW
0
...
"" ""
PENS
"pen-0" 1.0 2 -13791810 true "" "ask firms [ plotxy K P ]"
...
"" ""
PENS
"pen-0" 1.0 2 -16777216 true "" "ask firms [plotxy M P]"
